...
// ── AI Help action handlers (mirror of calculator.js; same row markup) ────
...
window.helpActionHandlers.highlight_bom_lines = function(params) {
  ...
  area.querySelectorAll('.parts-group').forEach(g => {
    ...
    if (!name) return;
    catBuckets.push({ name, rows: Array.from(g.querySelectorAll('.assembly-item')) });
  });
...
// ── AI Help context publishing (see calculator.js for the same helper) ────
...
function onBodyOptRadioChange(bid, groupKey) {
  ...
  // If this is an EPS/PU insulation switch, offer to apply globally
...
function _showInsulationSwitchModal(fromType, toType) {
  ...
  fresh.addEventListener('click', () => {
    ...
    bomData.forEach(it => {
      ...
      bodyOptionSelections[String(it.id)] = n.includes(toType);
    });
    saveBodyOptSel();
    renderBodyOptions(bomData);
    refreshBomDisplay();
...
// round-trip (same machinery as calculator.js; see the block comment there).
// ctxEditPermanent navigates the page away and the return trip reloaded the
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (fromId) {
    ...
    // WO v1.39.9 — flagged return trip from a permanent-price edit restores
    // the full in-progress costing instead of reloading trailer defaults.
...
async function loadBOM(options = {}) {
  ...
  try {
    ...
    saveCalc2Exclusions(tid, new Set(
      bomData.filter(r => r.calc2_default_excluded).map(r => r.id)
    ));
...
async function runCalc() {
  ...
  // Cost Calculator 2: bypass all body-option gating; the only client-side filter
  // is the per-row "x" tickbox set, persisted in localStorage per body type.
  //
  // Optional-section overlay (EXTRAS / OPTIONAL EXTRAS): merge any bom_ids
  // that are excluded because their section is disabled or because the user
  // unticked them inside an enabled optional section.
  const _exclC2 = loadCalc2Exclusions(+tid);
  // Optional-section per-row excludes are keyed by trailer_id in localStorage,
  // so they survive body-type switches — read directly rather than filter
  // through lastResult.items (which can belong to a different trailer the user
  // just visited). Section-disabled rows are default-excluded by the backend
  // via optional_sections_enabled, so we don't expand them here.
  if (window.OptionalSections && tid) {
    window.OptionalSections.loadRowExcl(+tid, 'c2').forEach(id => _exclC2.add(+id));
  }
  const _optEnabledIdsC2 = (window.OptionalSections && tid)
  ...
  lastCalcPayload = {
    ...
    body_option_selections: undefined,
    excluded_categories: [],
    flag_overrides: undefined,
...
async function approveCosting() {
  ...
  try {
    ...
    const isRepair  = !!_pendingApproveBase.is_repair;
    const dup = await api('GET', `/api/check-duplicate?customer_id=${customerId}&trailer_type_id=${trailerId}&is_repair=${isRepair}`);
...
// Calculator 2: per-trailer set of bom_ids the user has ticked off ('x') to
// exclude from the grand total. Persisted to localStorage per body type so the
// exclusions survive reloads and trailer-switches.
function _calc2ExclKey(tid) { return `calc2_excluded_bom_ids_${tid}`; }
function loadCalc2Exclusions(tid) {
  if (!tid) return new Set();
  try {
    const raw = localStorage.getItem(_calc2ExclKey(tid));
    return new Set((raw ? JSON.parse(raw) : []).map(Number).filter(Number.isFinite));
  } catch (_) { return new Set(); }
}
function saveCalc2Exclusions(tid, set) {
  if (!tid) return;
  try { localStorage.setItem(_calc2ExclKey(tid), JSON.stringify([...set])); } catch (_) {}
}
window.toggleCalc2Exclusion = function (bomId, checked) {
  const tidVal = document.getElementById('trailer-select')?.value;
  if (!tidVal) return;
  const tid = +tidVal;
  const set = loadCalc2Exclusions(tid);
  const id = +bomId;
  if (checked) set.add(id); else set.delete(id);
  saveCalc2Exclusions(tid, set);
  if (typeof scheduleCalc === 'function') scheduleCalc();
};

// Category header tickbox: when the user ticks it, every BOM row in that
// section gets added to (or removed from) the exclusion set, so the whole
// category drops out of the grand total in one click.
window.toggleCalc2CategoryExclusion = function (cb) {
  const tidVal = document.getElementById('trailer-select')?.value;
  if (!tidVal) return;
  const tid = +tidVal;
  const set = loadCalc2Exclusions(tid);
  ...
  if (cb.checked) ids.forEach(id => set.add(+id));
...
// Costings 2: master toggle for an EXTRAS / OPTIONAL EXTRAS section.
window.toggleOptionalSectionCalc2 = function (cb) {
  const tidVal = document.getElementById('trailer-select')?.value;
  ...
  let ids = [];
  try { ids = JSON.parse(decodeURIComponent(cb.dataset.bomIds || '[]')); } catch (_) { ids = []; }
  window.OptionalSections.toggleSection(tid, 'c2', sectionId, ids, !cb.checked);
  _rerenderBOMLocal();
  if (typeof scheduleCalc === 'function') scheduleCalc();
};

// Costings 2: per-row tick inside an enabled optional section.
window.toggleOptionalRowCalc2 = function (bomId, excluded, sectionId) {
  const tidVal = document.getElementById('trailer-select')?.value;
  if (!tidVal || !window.OptionalSections) return;
  const tid = +tidVal;
  ...
  window.OptionalSections.toggleRow(tid, 'c2', sectionId, ids, +bomId, !!excluded);
  _rerenderBOMLocal();
  if (typeof scheduleCalc === 'function') scheduleCalc();
};

// Costings 2: bulk select/deselect every row in an optional section.
window.bulkOptionalRowsCalc2 = function (ev, sectionId, idsAttr, selectAll) {
  if (ev) ev.stopPropagation();
  const tidVal = document.getElementById('trailer-select')?.value;
  if (!tidVal || !window.OptionalSections) return;
  let ids = [];
  try { ids = JSON.parse(decodeURIComponent(idsAttr || '[]')); } catch (_) { ids = []; }
  window.OptionalSections.bulkRows(+tidVal, 'c2', sectionId, ids, !!selectAll);
  _rerenderBOMLocal();
  if (typeof scheduleCalc === 'function') scheduleCalc();
};

// Costings 2: click-target version for the X / ✓ section badge.
window.setOptionalSectionCalc2 = function (ev, sectionId, idsAttr, enable) {
  if (ev) ev.stopPropagation();
  const tidVal = document.getElementById('trailer-select')?.value;
  if (!tidVal || !window.OptionalSections) return;
  let ids = [];
  try { ids = JSON.parse(decodeURIComponent(idsAttr || '[]')); } catch (_) { ids = []; }
  window.OptionalSections.toggleSection(+tidVal, 'c2', +sectionId, ids, !!enable);
  _rerenderBOMLocal();
  if (typeof scheduleCalc === 'function') scheduleCalc();
};
...
function renderBOMWithCosts(items, bomRef) {
  ...
  const _calc2Excl = loadCalc2Exclusions(tidNum);
  // Optional-section state — shared with Costings 1 via the helper.
  ...
  const _optRowExcl = (window.OptionalSections && tidNum)
    ? window.OptionalSections.loadRowExcl(tidNum, 'c2') : new Set();

  let html = '<table style="width:100%;font-size:12px;border-collapse:collapse">';
  html += `<thead><tr style="background:var(--bg-panel)">
    <th style="padding:6px 4px;text-align:center;color:var(--text-dim);font-size:10px;width:28px" title="Tick to exclude an item from the costing">✕</th>
  ...
  for (const [cat, its] of sortedEntries) {
    ...
    const catTotal   = its.reduce((s, it) => {
    ...
    // Calc2: excluded rows are user-driven and always visible — no eye toggle needed.
    const eyeBtn = '';

    // Category-level tickbox state: derived from the per-row exclusion set so
    // the header checkbox is always consistent with the individual row ticks.
    //   all rows in section excluded  → checked
    //   some rows excluded            → indeterminate (set in JS after render)
    //   none excluded                 → unchecked
    const _catBomIds = its.map(x => x.bom_id).filter(id => id != null);
    const _catExcludedCount = _catBomIds.filter(id => _calc2Excl.has(+id)).length;
    const _catAll = _catBomIds.length > 0 && _catExcludedCount === _catBomIds.length;
    const _catSome = _catExcludedCount > 0 && !_catAll;
    const _catIdsAttr = encodeURIComponent(JSON.stringify(_catBomIds));

    // Optional-section overlay: when is_optional is set, the header checkbox
    // flips semantics — it becomes a "tick to ENABLE this section" toggle
    // (sections start off / greyed). Red header + non-standard tooltip.
    ...
    const _optExcludedCount = _secOptional
      ? _catBomIds.filter(id => !_secEnabled || _optRowExcl.has(+id)).length
      : 0;
    const _optAll = _secOptional && _catBomIds.length > 0 && _optExcludedCount === _catBomIds.length;
    const _optSome = _secOptional && _optExcludedCount > 0 && !_optAll;
    const _hdrColor    = _secOptional ? 'var(--red,#e35d6a)' : 'var(--blue-hi)';
    const _hdrTitle    = _secOptional
      ? 'Non Standard items — tick checkbox to include the whole section'
      : 'Click row to collapse / expand · tick checkbox to exclude the whole section';
    ...
    const _hdrCheckbox = _secOptional
      ? `<input type="checkbox" class="opt-sec-tick" data-section-id="${_secId}" data-bom-ids="${_catIdsAttr}"
            ${_optAll ? 'checked' : ''}
            ${_optSome ? 'data-indeterminate="1"' : ''}
            onchange="toggleOptionalSectionCalc2(this)"
            title="${_optAll ? 'Untick to include every item in this section' : 'Tick to exclude every item in this section'}"
            style="cursor:pointer;width:14px;height:14px;accent-color:var(--red)"/>`
      : `<input type="checkbox" class="calc2-cat-tick" data-cat-name="${escHtml(cat)}" data-bom-ids="${_catIdsAttr}"
          ${_catAll ? 'checked' : ''}
          ${_catSome ? 'data-indeterminate="1"' : ''}
          onchange="toggleCalc2CategoryExclusion(this)"
          title="Tick to exclude every item in this section from the costing"
          style="cursor:pointer;width:14px;height:14px;accent-color:var(--red)"/>`;
    ...
    const _allRowsIncluded = _catBomIds.length > 0 && _secEnabled && _catBomIds.every(id => !_optRowExcl.has(+id));
    const _bulkSelectAll = !_allRowsIncluded;
    const _bulkBtn = _secOptional
      ? ` <button type="button"
            class="calc-bulk-pill-btn"
            onclick="bulkOptionalRowsCalc2(event, ${_secId}, '${_catIdsAttr}', ${_bulkSelectAll}); return false"
    ...
    html += `<tr class="calc-grp-hdr${collapsed ? ' collapsed' : ''}${_secOptional ? ' opt-sec-hdr' : ''}${_secOptional && !_secEnabled ? ' opt-sec-disabled' : ''}" data-cat-id="${gid}" data-cat-name="${escHtml(cat)}" data-section-id="${_secId != null ? _secId : ''}"
      ...
      <td style="padding:6px 4px;text-align:center;vertical-align:middle;width:28px;background:var(--bg-panel)" onclick="event.stopPropagation()">
        ${_hdrCheckbox}
      </td>
      <td colspan="4" style="padding:6px 8px;background:var(--bg-panel)">
        <span class="grp-chevron" style="font-size:10px;margin-right:5px;color:var(--text-dim)">${collapsed ? '▶' : '▼'}</span>
    ...
    its.forEach(it => {
      // Calc2: every row always renders so the user can toggle its tickbox.
      // Excluded rows are still styled struck-through + dimmed (see excludedStyle below).
      ...
      const excludedBadge = it.excluded && it.excluded_reason
        ...
        : (it.condition_summary
            ...
            : '');
      const _calc2Checked = bomRowId && _calc2Excl && _calc2Excl.has(+bomRowId);
      // Optional-section rows: the row checkbox is routed through the optional
      // helper (page='c2'). Always render the checkbox so the row reflects
      // ✓ included / ✗ excluded state whether the section is enabled or not.
      // When the section is disabled, every item shows unchecked + disabled.
      const _rowOptional   = !!it.section_is_optional;
      const _rowOptExcl    = _rowOptional && bomRowId && _optRowExcl && _optRowExcl.has(+bomRowId);
      const _rowOptIncluded = _rowOptional && _secEnabled && !_rowOptExcl;
      let _rowTickHtml;
      if (_rowOptional) {
        // Per-item tick — native red checkbox matching SRD-style.
        // Checked (red filled X) = EXCLUDED, unchecked = included.
        _rowTickHtml = `<input type="checkbox" class="opt-row-tick" data-bom-id="${bomRowId}"
            ${_rowOptIncluded ? '' : 'checked'}
            onchange="toggleOptionalRowCalc2(${bomRowId}, this.checked, ${_secId})"
            title="${_rowOptIncluded ? 'Tick to exclude this item' : 'Untick to include this item'}"
            style="cursor:pointer;width:14px;height:14px;accent-color:var(--red)"/>`;
      } else {
        _rowTickHtml = `<input type="checkbox" class="calc2-excl-tick" data-bom-id="${bomRowId}"
            ${_calc2Checked ? 'checked' : ''}
            onchange="toggleCalc2Exclusion(${bomRowId}, this.checked)"
            title="Tick to exclude this item from the costing"
            style="cursor:pointer;width:14px;height:14px;accent-color:var(--red)"/>`;
      }
      // Apply struck-through styling to optional rows whose section is off or
      // that are individually excluded (in addition to the existing condition-
      // excluded styling).
      const _optStyle = _rowOptional && (!_rowOptIncluded)
        ? 'text-decoration:line-through;opacity:.5;' : '';
      html += `<tr class="calc-grp-row${skinName ? ' bom-skin-row' : ''}${tapingName ? ' bom-taping-row' : ''}${floorName ? ' bom-floor-row' : ''}${cleatName ? ' bom-cleat-row' : ''}${it.excluded ? ' bom-excluded-row' : ''}${_rowOptional ? ' opt-sec-row' : ''}" data-cat-group="${gid}"
        ...
        <td style="padding:5px 4px;text-align:center;vertical-align:middle;width:28px" onclick="event.stopPropagation()">
          ${_rowTickHtml}
        </td>
  ...
  if (_ch && _ch.items && _ch.items.length) {
    ...
    const renderChassisGroup = (title, rows, opts = {}) => {
      ...
      let h = `<tr class="calc-grp-hdr${collapsed ? ' collapsed' : ''}" data-cat-id="${gid}" data-cat-name="${escHtml(title)}"
          ...
          style="cursor:pointer;user-select:none">
        <td colspan="5" style="padding:6px 8px;background:${bg};${indent}">
    ...
    if (constants.length) {
      ...
      html += `<tr><td colspan="5" style="padding:6px 12px;background:#1f1608">
  ...
  html += '</tbody></table>';
  area.innerHTML = html;
  // Apply indeterminate state to category checkboxes that had a mix of
  // excluded/included rows (HTML doesn't expose `indeterminate` as an attribute).
  area.querySelectorAll('.calc2-cat-tick[data-indeterminate="1"], .opt-sec-tick[data-indeterminate="1"]').forEach(cb => { cb.indeterminate = true; });
