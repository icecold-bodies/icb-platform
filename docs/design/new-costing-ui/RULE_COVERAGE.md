...
| CALC-004 Token resolution + `formula_error` | **DESIGN** | Row state *formula-error*: red qty "— err —", tag "formula error · unknown {TOKEN}" + "how to fix ›", red left stripe, counted in the R1 attention pill; save requires acknowledgement (D3). **Correction paths (mocked):** (1) quick — click the red cell and type the quantity for this costing (override bypasses the broken formula; tag "formula error bypassed · qty typed"; ⚠ clears); (2) proper — ⋯ → Edit formula → fix on the body template (gated, journalled; refuses to save while the unknown token remains); (3) define the missing token as a variable in Admin → Formulas (deep link). The line no longer *silently* costs the zero-substituted result. |
...
| SEC-010 Section multiplier | **DESIGN** | `× N` badge on the card header. **Single-side view REINSTATED (Michael, 17 Aug — OQ-10 revisited):** clicking the badge shows every multiplied line and the totals at ONE side (badge `× 1`, amber "½ ONE SIDE VIEW — click elsewhere to reset" on the card and in the rail); whole-costing like today; display-only, never saved or exported; reference drift is not evaluated while it is on. Nadie uses it to tick lines against the workbook's single-side SIDES section. |
