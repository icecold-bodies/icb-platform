...
- **All money at 2 dp** in the app's SA money style (`R 3 795,74`, `R 109 540,22`): unit prices first, then line totals — with category subtotals, the rail index, the chassis block and the totals bar following, so the visible lines add up to the visible subtotal. Typed prices accept a comma decimal.
